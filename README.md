...
- **`HybridPTSampler`** — parallel tempering interleaved with NUTS and birth/death model moves
...
`PTSampler.from_product_space` wires up the trans-dimensional kernels: one **combined
...
sampler = PTSampler.from_product_space(space, ntemps=8, seed=42, outdir="./chains_rj")
