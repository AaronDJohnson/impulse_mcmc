...
class TestVectorizedMhStep:
    ...
    def test_vectorized_mh_step_acceptance_logic(
        self, sample_state_2d, simple_likelihood, simple_prior
    ):
    ...
    def test_vectorized_mh_step_rejection(self, sample_state_2d, simple_likelihood, simple_prior):
        ...
        if np.any(rejected_mask):
            np.testing.assert_array_equal(
                new_state.positions[rejected_mask], sample_state_2d.positions[rejected_mask]
            )

    def test_vectorized_mh_step_proposal_ratios(
        self, sample_state_2d, simple_likelihood, simple_prior
    ):
    ...
    def test_vectorized_mh_step_temperature_scaling(
        self, sample_state_2d, simple_likelihood, simple_prior
    ):
    ...
    def test_vectorized_mh_step_inf_temp_accepts_neg_inf_likelihood(self, simple_prior):
        ...
        state = SamplerState(positions, lnlikes, lnpriors, lnprobs, np.ones(2, dtype=int), temps)
    ...
    def test_vectorized_mh_step_updates_probabilities(
        self, sample_state_2d, simple_likelihood, simple_prior
    ):
        ...
        if np.any(accepted_mask):
            ...
            assert not np.allclose(
                new_state.lnlikes[accepted_mask], sample_state_2d.lnlikes[accepted_mask]
            )
