...
class TestFunctionWrapper:
    ...
    def test_call_nonzero_leading_dim(self):
        ...
        with pytest.raises(
            ValueError, match="Input to _function_wrapper must have nonzero leading dimension"
        ):
    ...
    def test_vectorized_wrong_output_shape(self):
        ...
        with pytest.raises(
            ValueError, match="Vectorized function returned array with incorrect leading dimension"
        ):
            wrapper(input_data)
    ...
    def test_extra_args_application(self):
        ...
        wrapper = _function_wrapper(func_with_multiple_args, args=(10, 20), kwargs={"c": 5})
    ...
    def test_int_array_output_with_inf(self):
        ...
        assert np.issubdtype(result.dtype, np.floating)
        np.testing.assert_array_equal(result, np.array([[0.0, 0.0], [-np.inf, -np.inf]]))
