...
class _PTSamplerBase:
    ...
    # names ("impulse.samplers" / "impulse.experimental.hybrid_sampler"); class attribute,
    ...
    def _migrate_or_warn_legacy_birth_death(self) -> None:
        """Repair pre-fix birth/death wiring restored from a legacy checkpoint.

        This is a legacy-compatibility shim, not core functionality. The core
        sampler carries no model-selection knowledge; all it does here is look
        for two proposal ``__name__``\\ s that only a product-space run can
        have, and defer to :mod:`impulse.experimental` when it finds them. The
        import is deliberately inside the branch, so importing the core sampler
        never pulls in the experimental package.

        The check cannot live in the experimental sampler alone:
        :func:`~impulse.experimental.make_product_space_sampler` builds a plain
        :class:`~impulse.PTSampler` by default, and a legacy checkpoint from
        such a run needs the same repair. See
        :func:`impulse.experimental._product_space_setup.migrate_or_warn_legacy_birth_death`
        for what the repair does and why the wiring was wrong.
        """
        ...
        from impulse.experimental._product_space_setup import (
            migrate_or_warn_legacy_birth_death,
        )

        migrate_or_warn_legacy_birth_death(self)
