...
import ray
from impulse.mhsampler import MHSampler, PMHSampler
...
def parallel_pt_sample(lnlike, lnprior, ndim, x0, num_samples=1_000_000, buf_size=50_000,
    ...
    # make set of samplers (1 for each temp)
    samplers = [PMHSampler.remote(x0[ii], lnlike, lnprior, mixes[ii], iterations=swap_count, init_temp=ptswap.ladder[ii]) for ii in range(ntemps)]
